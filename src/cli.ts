...
program
  ...
  .option('--input-tokens <n>', 'Input token count (raw, excluding cache)')
  .option('--output-tokens <n>', 'Output token count')
  .option('--cache-read-tokens <n>', 'Cache-read input token count')
  .option('--cache-write-tokens <n>', 'Cache-write input token count')
